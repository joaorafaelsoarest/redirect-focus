...
export function normalizeCurrentSite(value) {
  let url;
  try {
    url = new URL(String(value ?? '').trim());
  } catch {
    throw new Error('Esta página não pode ser classificada.');
  }
  ...
  return { domain, productiveUrl: `${url.protocol}//${domain}${port}` };
}
...
export function classifyCurrentSite(configuration, value, category) {
  let site;
  try {
    site = normalizeCurrentSite(value);
  } catch (error) {
    return { ok: false, error: error.message };
  }
  if (!['blocked', 'productive'].includes(category)) {
    return { ok: false, error: 'Categoria inválida.' };
  }

  const blockedDomains = Array.isArray(configuration?.blockedDomains) ? configuration.blockedDomains : [];
  const productiveUrls = Array.isArray(configuration?.productiveUrls) ? configuration.productiveUrls : [];
  const overlapsWithSite = (candidate) => {
    try { return domainsOverlap(site.domain, candidate); } catch { return false; }
  };
  const overlapsProductiveSite = (value) => {
    try { return overlapsWithSite(new URL(normalizeProductiveUrl(value)).hostname); } catch { return false; }
  };
  const alreadyBlocked = blockedDomains.some(overlapsWithSite);
  const alreadyProductive = productiveUrls.some(overlapsProductiveSite);
  const nextBlockedDomains = category === 'productive'
    ? blockedDomains.filter((domain) => !overlapsWithSite(domain))
    : alreadyBlocked ? [...blockedDomains] : [...blockedDomains, site.domain];
  const nextProductiveUrls = category === 'blocked'
    ? productiveUrls.filter((url) => !overlapsProductiveSite(url))
    : alreadyProductive ? [...productiveUrls] : [...productiveUrls, site.productiveUrl];
  const nextConfiguration = { blockedDomains: nextBlockedDomains, productiveUrls: nextProductiveUrls };
  const changed = JSON.stringify(nextConfiguration.blockedDomains) !== JSON.stringify(blockedDomains)
    || JSON.stringify(nextConfiguration.productiveUrls) !== JSON.stringify(productiveUrls);
...
export function remainingPauseMinutes(until, now = Date.now()) {
  const pauseUntil = Number(until);
  if (!Number.isFinite(pauseUntil) || pauseUntil <= now) return null;
  return Math.ceil((pauseUntil - now) / 60_000);
}
